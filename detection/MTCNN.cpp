...
void MTCNN::detection(const cv::Mat& img, std::vector<cv::Rect>& rectangles)
{
    ...
    O_Net();
    global_NMS();
    rectangles = bounding_box_;
}


void MTCNN::detection(const cv::Mat& img, std::vector<cv::Rect>& rectangles, std::vector<cv::Rect> rect_lastframe)
{
    Preprocess(img);
    P_Net();
    local_NMS();
    R_Net();
    local_NMS();
    for(int i = 0; i < rect_lastframe.size(); i++)
        bounding_box_.push_back(rect_lastframe[i]);
    O_Net();
    global_NMS();
    rectangles = bounding_box_;
}
